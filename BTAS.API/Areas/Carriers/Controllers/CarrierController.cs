...
namespace BTAS.API.Areas.Carriers.Controllers
{
    ...
    public class CarrierController : ControllerBase
    {
        ...
        public CarrierController(IConfiguration configuration, ApgRepository repo, IMapper mapper)
        {
        ...
        {
            var client = CallApg("POST", "/services/shipper/labels");
            ...
            using (WebResponse response = await client.GetResponseAsync())
            {
                ...
                if (result.IndexOf("Failure") > -1)
                {
                    return new JsonResult(JsonConvert.DeserializeObject<CreateLabelResponseError>(result));
                }
                else
                {
                    return new JsonResult(JsonConvert.DeserializeObject<CreateLabelResponseSuccess>(result));
                }
            }
        }
