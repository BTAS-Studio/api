...
namespace BTAS.API.Areas.Waybill.Controllers
{
    ...
    public class ShipmentController : GenericController<tbl_shipmentDto>
    {
        ...
        [HttpPost]
        [Route("create")]
        public async Task<IActionResult> CreateAsync([FromBody] tbl_shipmentDto request)
        {
            try
            {
                var result = await _repository.CreateAsync(request); //tbl_shipmentDto
                ...
                if (result != null)
                {
                    ...
                    if (model.tbl_shipment_serviceCode.ToUpper().IndexOf("UBI") > -1)
                    {
                        ...
                        foreach (var item in request.shipmentItems)
                        {
                            ...
                            {
                                var skuCode = await _skuRepository.GetByReferenceAsync(sku.tbl_item_sku_code);
        ...
        public async Task<IActionResult> UpdateAsync([FromBody] tbl_shipmentDto request)
        {
                ...
                try
                {
                    ...
                    return new JsonResult(new GeneralResponse
                    {
                        success = false,
                        response = 500,
                        responseDescription = "Missing/Invalid shipment reference."
                    });
                }
                catch (Exception ex)
                {
