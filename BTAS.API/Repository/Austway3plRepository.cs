...
{
    public class Austway3plRepository
    {
        private readonly MainDbContext _context;
        private IMapper _mapper;
        private IAustwayLabelRepository repo;

        public Austway3plRepository(MainDbContext context, IMapper mapper, IAustwayLabelRepository repo)
        {
            _context = context;
            _mapper = mapper;
            this.repo = repo;
        ...
        public async Task<CreateLabelResponse> CreateLabelAsync(List<LabelItem> items, tbl_addressDto recipient, tbl_client_headerDto shipper = null)
        {
            try
            {
                var routing = _context.tbl_3pl_routings
                ...
                var consignment = new tbl_tracking_3pl
                {
                ...
                _context.tbl_tracking_3pls.Add(consignment);
                await _context.SaveChangesAsync();

                int i = 0;
                foreach (var item in items)
                {
                    string tracking = "APL" + String.Format("{0:000000000}", consignment.tbl_tracking_id) + String.Format("{0:0000}", i + 1);
                    ...
                    items[i].Consignment = "APL" + String.Format("{0:00000000}", consignment.tbl_tracking_id);

                    i++;
                }

                return await repo.GenerateLabelAsync("Austway 3PL", items, recipient, shipper);
            }
            catch (Exception ex)
            {
                return new CreateLabelResponse
